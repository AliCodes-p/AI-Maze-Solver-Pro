...
# ═══════════════════════════════════════════════════════
#  REAL MAP ROUTING  (OpenStreetMap + OSRM — 100% FREE)
# ════════════════════════════════════
...
class MazeSolverPro:
    ...
    #  GRID
    # ───────────────────────────────────────────────────
    def _cell_xy(self, r, c): #return coordinates to tel where to draw rectangle 
        x1 = PADDING + c*(CELL_SIZE+PADDING)
        y1 = PADDING + r*(CELL_SIZE+PADDING)
        return x1, y1, x1+CELL_SIZE, y1+CELL_SIZE

    def _draw_grid(self): #crates the whole grid 
    ...
    def _set_cell(self, r, c, fill, outline=None): #chane the colour of the specific cell
        rid = self.cells.get((r,c))
        if rid:
            self.canvas.itemconfig(rid, fill=fill,
                                   outline=outline or CLR_GRID)

    def _redraw_cell(self, r, c): #restore correct appearance of a cell based on its state (start, end, wall, or empty)
        if (r,c)==self.start:
    ...
    def _put_label(self, r, c, txt): #write text inside a label (cell)
        x1,y1,x2,y2 = self._cell_xy(r,c)
    ...
    def _sync_labels(self): #it refresh all the labels (start and end) to ensure they are correctly displayed after any changes to the grid (like adding/removing walls or changing start/end points)
    ...
    # ═══════════════════════════════════════════════════
    #  DFS  — Deep search (may not be shortest)
    # ═══════════════════════════════════════════════════
    def _run_dfs(self):
        t0 = time.time()
        start, end = self.start, self.end
    ...
    def _open_interactive_map(self):
        ...
        def run():
            ...
            # Get actual road routes for visual display
